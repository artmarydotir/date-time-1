...
class AasaamDateTime {
  ...
  */
  yearOffset(calendar, offset) {
    ...
    if (calendar === 'persian') {
      this.date = momentJ(this.date).jYear(year + offset).toDate();
    } else if (calendar === 'islamic') {
      this.date = momentH(this.date).iYear(year + offset).toDate();
    } else {
      this.date = momentG(this.date).year(year + offset).toDate();
    }
  ...
  */
  monthOffset(calendar, offset) {
    ...
    if (calendar === 'persian') {
      this.date = momentJ(this.date).jMonth(month + offset).toDate();
    } else if (calendar === 'islamic') {
      this.date = momentH(this.date).iMonth(month + offset).toDate();
    } else {
      this.date = momentG(this.date).month(month + offset).toDate();
    }
  ...
  */
  dayOffset(calendar, offset) {
    let current;
    if (calendar === 'persian') {
      current = momentJ(this.date).locale('en').format('jD');
    } else if (calendar === 'islamic') {
      current = momentH(this.date).locale('en').format('iD');
    } else {
      current = momentG(this.date).locale('en').format('D');
    }

    const day = parseInt(current, 10);

    if (calendar === 'persian') {
      this.date = momentJ(this.date).jDate(day + offset).toDate();
    } else if (calendar === 'islamic') {
      this.date = momentH(this.date).iDate(day + offset).toDate();
    } else {
      this.date = momentG(this.date).date(day + offset).toDate();
    }
